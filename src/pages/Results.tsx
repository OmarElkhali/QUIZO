    ...
    return (
      <AppShell>
        <StateCard
          ...
          action={<Button className="quizo-copper-button" onClick={() => navigate('/')}>{t('results.backToHome')}</Button>}
        />
      </AppShell>
    );
  }
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        title={quiz.title}
        description={t('results.recapDesc')}
        actions={
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <PremiumPanel className="p-6 text-center">
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            <PremiumMetric label={t('results.correct')} value={correctAnswers} icon={Check} tone="green" className="p-4" />
            <PremiumMetric label={t('results.incorrect')} value={answeredIncorrectly} icon={X} tone="default" className="p-4" />
            <PremiumMetric label={t('results.unanswered')} value={unansweredQuestions} icon={Target} tone="copper" className="p-4" />
        ...
        <PremiumPanel className="overflow-hidden">
          <div className="border-b border-white/[0.07] p-6">
            <h2 className="text-2xl font-bold text-white">{t('results.viewDetails')}</h2>
            <p className="mt-1 text-sm text-[#a79d96]">{t('results.filterDesc')}</p>
          </div>
          <div className="p-6">
            <Tabs defaultValue="all" onValueChange={setActiveTab}>
              <TabsList className="mb-6 grid h-auto grid-cols-2 gap-2 bg-transparent md:grid-cols-4">
                ...
                <TabsTrigger value="correct" className="border border-white/[0.07] bg-white/[0.045] data-[state=active]:bg-emerald-500/15 data-[state=active]:text-emerald-300">
                  {t('results.correct')} ({correctAnswers})
                </TabsTrigger>
                <TabsTrigger value="incorrect" className="border border-white/[0.07] bg-white/[0.045] data-[state=active]:bg-red-500/15 data-[state=active]:text-red-300">
                  {t('results.incorrect')} ({answeredIncorrectly})
                </TabsTrigger>
                <TabsTrigger value="unanswered" className="border border-white/[0.07] bg-white/[0.045] data-[state=active]:bg-orange-500/15 data-[state=active]:text-[#ffb77d]">
                  {t('results.unanswered')} ({unansweredQuestions})
                </TabsTrigger>
              </TabsList>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-4 space-y-2 border-l border-white/10 pl-4 text-sm text-[#d8d2ce]">
                            <p>
                              ...
                              <span>{userAnswer ? userAnswer.text : t('results.noAnswer')}</span>
                            </p>
                            {!isCorrect && (
